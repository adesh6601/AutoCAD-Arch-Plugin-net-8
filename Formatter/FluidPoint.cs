...
public class FluidPoint
{
    public class PriorityQueue<TPriority, TValue>
    {
        ...
        public bool TryDequeue(out TPriority priority, out TValue value)
        {
            ...
            {
                var kvp = _dictionary.First();
                priority = kvp.Key;
                value = kvp.Value.Dequeue();
    ...
    public class FluidPointCalculator
    {
        ...
        public static float[] GetPolyLabel(float[][][] polygon, float precision = 1f)
        {
            ...
            {
                throw new Exception("\nError encountered while calculating fluidpoint of a room. " + ex.Message);
            }
        }
        ...
        public static float[][] ConvertPolygonToFloatArray(List<Point> points_in_polygon)
        {
            // this is a float[][]
            var polygon = new float[points_in_polygon.Count][];

            var pointCount = 0;
            foreach (Point point in points_in_polygon)
            {
                polygon[pointCount] = new float[2];
                polygon[pointCount][0] = (float)point.X;
                polygon[pointCount][1] = (float)point.Y;
                pointCount++;
            }
